...
class COL761LinkDataset:
    ...
    def __init__(self, ds_dir: str):
        ...
        if ref_pos is not None:
            ref_edge = ref_pos.t()
            self.edge_index = torch.cat([ref_edge, ref_edge[[1, 0]]], dim=1)
        else:
...
def _print_stats(dataset: str, ds) -> None:
    ...
    if isinstance(ds, COL761NodeDataset):
        ...
        print(f"  {ds}")
        for attr in ("train_pos", "train_neg", "valid_pos", "valid_neg", "test_pos", "test_neg"):
            if hasattr(ds, attr):
...
def main():
    parser = argparse.ArgumentParser(description="Load COL761 A3 assignment datasets A, B, or C.")
