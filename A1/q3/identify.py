...
def parse_graphs(file_path):
    #TODO: Why not return in gSpan format itself?
    """
...
def write_gspan_format(graphs, output_path):
    ...
    #TODO: why do i return output_path here?
...
def gspan_to_nx(g):
    ...
    return G
    #TODO: what kind of graph structure is used in both gspan and networkx?

def main():
    if len(sys.argv) < 3:
        print("Usage: python identify.py <input_graphs> <output_subgraphs>")
        sys.exit(1)
    ...
    temp_gspan_file = "temp_gspan_input.data"
    ...
    # select features close to 50% support for better variance?
    # p(1-p) is max at 0.5.
    # Let's calculate discriminative score: -|support_ratio - 0.5| (max is 0)
    
    total_graphs = len(graphs)
    
    # Add discriminative score
    scored = []
    for sup, g in subgraphs_with_support:
        ratio = sup / total_graphs
        score = -abs(ratio - 0.5) 
        scored.append((score, g))
    
    # Sort by score descending (closest to 50%)
    scored.sort(key=lambda x: x[0], reverse=True)
    
    k = 50
    selected = [x[1] for x in scored[:k]]
    
    print(f"Selected {len(selected)} discriminative subgraphs.")
