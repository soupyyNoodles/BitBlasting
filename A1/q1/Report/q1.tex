...
Both algorithms were intended to be executed at five different support thresholds: 5\%, 10\%, 25\%, 50\%, and 90\%. The aggregated wall-clock times for these runs are taken from \texttt{A1/q1/output\_q1\_1\_logs/results.txt}.

Since WebDocs is very large, 5\% is typically the most expensive regime. In our 5\% run, Apriori exceeded the runtime budget and timed out (recorded as a \textasciitilde3600s cap in \texttt{results.txt}), leaving an empty output file. FP-Growth, however, completed at 5\% support; its output file (\texttt{fp5}) was not committed to the repository due to large size, but the end-to-end runtime is still available from \texttt{results.txt}.
...
\hline
5 & timeout ($\ge$3600.12) & 189.67 & --- \\
\hline
10 & 631.56 & 125.73 & 5.02\texttimes \\
\hline
25 & 39.71 & 39.31 & 1.01\texttimes \\
\hline
50 & 35.75 & 35.17 & 1.02\texttimes \\
\hline
90 & 34.33 & 34.24 & 1.00\texttimes \\
\hline
\end{tabular}
...
\noindent\textbf{Note:} At 5\% support, Apriori timed out; therefore a direct speedup value is not reported for that row.
...
\hline
5  & Apriori timeout; FP completed & 683 & 1,394,053 / 1,692,082 & 2,590,180 & --- \\
\hline
...
\begin{enumerate}
    \item \textbf{5\% is extremely hard for Apriori:} At 5\% support, Apriori exceeded the time budget and timed out (\textasciitilde3600s cap), while FP-Growth still completed in 189.67s. This highlights how quickly Apriori's candidate/subset-checking workload can blow up at low supports.

    \item \textbf{Dramatic speedup in the low-support regime (completed runs):} At 10\% support, FP-Growth is about 5.02\texttimes faster than Apriori (125.73s vs 631.56s).
...
\begin{itemize}
    \item \textbf{Apriori:} reading 35.89s; filtering 2.08s; reduction 4.01s; tree build 0.68s; subset checking 588.59s; output write 0.04s.
    \item \textbf{FP-Growth:} reading 34.61s; filtering 1.73s; reduction 4.23s; output write (includes mining) 83.71s.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{25\% (transition):} only 29 items survive filtering and only 413 frequent itemsets remain; Apriori now explores only up to size 6. The expensive part collapses sharply, and both algorithms become dominated by reading/parsing and preprocessing the large WebDocs file.
    ...
    \item \textbf{90\% (early termination):} both tools report \emph{no frequent items found} right after reading the dataset. Even when the mining result is empty, there is still a fixed cost to stream and count item occurrences in the input, hence the runtime remains on the order of tens of seconds despite producing no itemsets.
\end{itemize}
...
Figure~\ref{fig:webdocs_plot} shows the runtime curve produced by the pipeline for the measured thresholds (including the 5\% FP-Growth datapoint and the 5\% Apriori timeout cap).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.80\linewidth]{../output_q1_1/plot.png}
    \caption{Runtime comparison on WebDocs (thresholds from \texttt{output\_q1\_1\_logs/results.txt}). X-axis: support threshold (\%). Y-axis: runtime (seconds).}
...
\begin{enumerate}
    \item \textbf{Consistent FP-Growth advantage:} FP-Growth maintains a significant speedup (6-8$\times$) across support levels 5-50\% on the synthetic dataset, compared to about 5.02$\times$ at 10\% on WebDocs (completed run).
    
    \item \textbf{Different speedup profiles:}
    \begin{itemize}
        \item \textbf{WebDocs:} Speedup decreases sharply as support increases (\textasciitilde5.02$\times$ at 10\% $\to$ \textasciitilde1.00$\times$ at 90\%).
