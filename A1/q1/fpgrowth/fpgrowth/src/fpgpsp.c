/*----------------------------------------------------------------------
  ...
  History : 2015.08.28 file created
            ...
            2024.12.12 bug in error reporting fixed
----------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define DESCRIPTION "generate or estimate a pattern spectrum " \
                    "(FP-growth)"
#define VERSION     "version 1.3 (2024.12.12)         " \
                    "(c) 2015-2024   Christian Borgelt"
...
int fpg_genpsp (TABAG *tabag, int target, double supp,
                ITEM zmin, ITEM zmax, int algo, int mode,
                size_t cnt, int surr, long seed,
                int cpus, PRGREPFN *rep, void* data, PATSPEC **pps)
{                               /* --- generate a pattern spectrum */
  ...
  if (!fpgrowth) return E_NOMEM;/* create an fpgrowth miner */
  r = (surr == FPG_SHUFFLE) ? FPG_NORECODE|FPG_NOSORT : 0;
  r = fpg_data(fpgrowth, tabag, r|FPG_NOPACK, +2); /* prepare the */
  if (r) { fpg_delete(fpgrowth, 0); return r; }    /* transactions */

  /* --- generate pattern spectrum --- */
  if (cpus <= 0) cpus = cpucnt();
  if ((cpus > 1) && (cnt > 1)){ /* if to use multi-threading */
    /* create array of thread handles */
    threads = (THREAD*)calloc((size_t)cpus, sizeof(THREAD));
    if (!threads) { fpg_delete(fpgrowth, 0); return E_NOMEM; }
    w = (WORKDATA*)calloc((size_t)cpus, sizeof(WORKDATA));
    if (!w) { free(threads); fpg_delete(fpgrowth, 0); return E_NOMEM; }
    ...
    if (!report) { fpg_delete(fpgrowth, 0); return E_NOMEM; }
    if ((fpg_report(fpgrowth, report) < 0)
    ||  (isr_addpsp(report, NULL)     < 0)
    ||  (isr_setup (report) != 0)) {
      isr_delete(report, 0); fpg_delete(fpgrowth, 0); return E_NOMEM; }
  ...
  /* --- clean up --- */
  #ifdef FPG_ABORT              /* if a signal handler is present */
  if (sig_aborted()) { sig_remove(); return E_ABORTED; }
  #endif
  if (r < 0) { if (psp) psp_delete(psp); return r; }
  #ifdef FPG_ABORT              /* if a signal handler is present */
  sig_remove();                 /* remove the signal handler */
  #endif
  *pps = psp;                   /* set created pattern spectrum */
  return 0;                     /* return 'OK' */
}  /* fpg_genpsp() */
...
int fpg_estpsp (TABAG *tabag, int target, double supp,
                ITEM zmin, ITEM zmax, size_t equiv,
                double alpha, size_t smpls, long seed, PATSPEC **pps)
{                               /* --- estimate a pattern spectrum */
  ...
  if (r) return r;              /* and check for an error */
  ...
  if (!psp) return E_NOMEM;     /* create a pattern spectrum */
  if (psp_tbgest(tabag, psp, (size_t)equiv, alpha, (size_t)smpls) != 0){
    psp_delete(psp); return E_NOMEM; } /* estimate pattern spectrum */
  *pps = psp;                   /* set created pattern spectrum */
  return 0;                     /* return 'OK' */
}  /* fpg_estpsp() */
...
{                               /* --- main function */
  ...
  if (surr < 0) {               /* if to estimate a pattern spectrum */
    ...
    k = fpg_estpsp(tabag, target, supp, zmin, zmax,
                   (size_t)cnt, alpha, (size_t)smpls, seed, &psp); }
  else {                        /* if to generate a pattern spectrum */
    MSG(stderr, "generating pattern spectrum ... ");
    k = fpg_genpsp(tabag, target, supp, zmin, zmax,
                   algo, mode, (size_t)cnt, surr, seed,
                   cpus, repfn, &done, &psp);
  }                             /* (create a pattern spectrum) */
  if (k) error(k);              /* check for an error */
